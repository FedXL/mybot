...
from utils.config import supported_channel, supported_bot, engine
from utils.posts_bot import send_message, delete_message, edit_message
...
async def catch_post_text(message: Message, state):
    ...
    await bot.send_message(message.from_user.id, message.html_text, reply_markup=GetKeyboard.proof,
...
async def create_post_finally(callback: CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        text = data['post_text']
        name = data['post_name']
        id_1 = data['first_message']
        id_2 = data['second_message']
        id_3 = data['third_message']
    ...
    message_id = send_message(supported_channel, text, supported_bot)
...
def register_handlers_critical_messages(dp: Dispatcher):
    ...
    dp.register_message_handler(catch_post_text, state=Admin.admin_critical_messages_text)
    ...
    dp.register_callback_query_handler(edit_post_menu, lambda c: c.data in ('edit_post'),
                                       state=Admin.admin_faq_channels)
    dp.register_callback_query_handler(make_choice_delete_or_edit, Regexp('post_([0-9])*'),
                                       state=Admin.admin_faq_channels)
